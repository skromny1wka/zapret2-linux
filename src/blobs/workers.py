...
class BlobsLoadWorker(QThread):
    ...
    def __init__(
        ...
        *,
        ...
        self._get_blobs_info = get_blobs_info
        self._reload_blobs = reload_blobs
        self._reload = bool(reload)

    def run(self) -> None:
        try:
            if self._reload:
                blobs_info = self._reload_blobs()
            else:
                blobs_info = self._get_blobs_info()
...
class BlobActionWorker(QThread):
    ...
    def __init__(
        ...
        *,
        ...
        self._save_user_blob = save_user_blob
        self._delete_user_blob = delete_user_blob
    ...
    def run(self) -> None:
        ...
        try:
            if self._action == "save":
                ...
                result = bool(self._delete_user_blob(self._name))
            else:
...
class BlobOpenActionWorker(QThread):
    ...
    def __init__(
        ...
        *,
        ...
        self._request_id = int(request_id)
        self._action = str(action or "").strip()
        self._open_bin_folder = open_bin_folder
        self._open_blobs_json = open_blobs_json

    def run(self) -> None:
        try:
            if self._action == "bin_folder":
                self._open_bin_folder()
            elif self._action == "blobs_json":
                self._open_blobs_json()
            else:
